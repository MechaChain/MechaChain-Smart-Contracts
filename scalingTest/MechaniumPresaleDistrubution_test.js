...
contract('MechaniumPresaleDistribution', (accounts) => {
  ...
  it(`Admin should be able to allocate tokens to ${accounts.length - 1} users`, async () => {
  ...
  it('Admin should claim all users tokens', async () => {
    const oneClaimResponse = await instance.claimTokens(accounts[accounts.length - 1]);
  ...
  it('ClaimAll gas usage must be lower than claim one', async () => {
    ...
    const priceMultipleOneClaims = parseFloat(priceOneClaim) * (accounts.length - 2);
    console.log(`One claim price x ${accounts.length - 2} : ${priceMultipleOneClaims} MATIC`);
    
    // Calculate price for claimForAll
    const gasWeiMultipleClaims = getBN(gasUsageMultipleClaims).mul(estimatedGasPrice);
    const priceMultipleClaims = web3Utils.fromWei(gasWeiMultipleClaims, 'ether');
    console.log(`Multiple claims function price : ${priceMultipleClaims} MATIC`);

    assert(parseFloat(priceMultipleClaims) < parseFloat(priceMultipleOneClaims));
  });
