...
contract Filterer is OperatorFilterer, Ownable {
    ...
    function testFilter(
        address from
    ) public view onlyAllowedOperator(from) returns (bool) {
        return true;
    }
}
...
contract OperatorFilterRegistryTest is BaseRegistryTest {
    ...
    function testRegisterAndSubscribe_NotRegistered() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                NotRegistered.selector,
                makeAddr("subscription")
            )
        );
        registry.registerAndSubscribe(address(this), makeAddr("subscription"));
    }

    function testRegisterAndSubscribe_CannotSubscribeToRegistrantWithSubscription()
        public
    {
        address subscription = makeAddr("subscription");
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                CannotSubscribeToRegistrantWithSubscription.selector,
                subscription
            )
        );
        registry.registerAndSubscribe(address(this), subscription);
    ...
    function testRegisterAndCopyEntries() public {
        ...
        registry.updateCodeHash(
            address(this),
            bytes32(bytes4(0xdeadbeef)),
            true
        );
        ...
        assertTrue(registry.isRegistered(address(filterer)));
        assertEq(
            registry.filteredOperatorAt(address(filterer), 0),
            makeAddr("operator")
        );
        assertEq(
            registry.filteredCodeHashAt(address(filterer), 0),
            bytes32(bytes4(0xdeadbeef))
        );
    }
    ...
    function testRegisterAndCopyEntries_AlreadyRegistered() public {
        ...
        registry.registerAndCopyEntries(
            address(this),
            makeAddr("not registered but fail fast")
        );
    }
    ...
    function testUpdateOperator() public {
        ...
        registry.updateOperator(address(this), makeAddr("operator"), true);
        assertTrue(
            registry.isOperatorFiltered(address(this), makeAddr("operator"))
        );
        assertEq(
    ...
    function testUpdateOperator_notRegistered() public {
        vm.expectRevert(
            abi.encodeWithSelector(NotRegistered.selector, address(this))
        );
        registry.updateOperator(address(this), makeAddr("operator"), true);
    }
    ...
    function testUpdateOperator_CannotUpdateWhileSubscribed() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                CannotUpdateWhileSubscribed.selector,
                subscription
            )
        );
        registry.updateOperator(address(this), makeAddr("operator"), true);
    ...
    function testUpdateOperator_AddressAlreadyFiltered() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AddressAlreadyFiltered.selector,
                makeAddr("operator")
            )
        );
        registry.updateOperator(address(this), makeAddr("operator"), true);
    ...
    function testUpdateCodeHash_CodeHashAlreadyFiltered() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                CodeHashAlreadyFiltered.selector,
                bytes32(bytes4(0xdeadbeef))
            )
        );
        registry.updateCodeHash(
    ...
    function testSubscribe_CannotSubscribeToZeroAddress() public {
        registry.register(address(this));
        vm.expectRevert(
            abi.encodeWithSelector(CannotSubscribeToZeroAddress.selector)
        );
    ...
    function testSubscribe_AlreadySubscribed() public {
        ...
        registry.subscribe(address(this), subscription);
        vm.expectRevert(
            abi.encodeWithSelector(AlreadySubscribed.selector, subscription)
        );
        registry.subscribe(address(this), subscription);
    }
    ...
    function testSubscribe_SubscriptionNotRegistered() public {
        ...
        address subscription = makeAddr("subscription");
        vm.expectRevert(
            abi.encodeWithSelector(NotRegistered.selector, subscription)
        );
        registry.subscribe(address(this), subscription);
    }
    ...
    function testSubscribe_CannotSubscribeToRegistrantWithSubscription()
        public
    {
        address subscription = makeAddr("subscription");
        address superSubscription = makeAddr("superSubscription");
    ...
    function testCopyEntriesOf() public {
        ...
        assertEq(
            registry.filteredOperators(address(this))[0],
            duplicateOperator
        );
        assertEq(
            registry.filteredOperatorAt(address(this), 0),
        ...
        assertEq(
            registry.filteredCodeHashes(address(this))[0],
            duplicateCodeHash
        );
        assertEq(
            registry.filteredCodeHashAt(address(this), 0),
    ...
    function testIsCodeHashOfFiltered() public {
        ...
        assertFalse(
            registry.isCodeHashOfFiltered(
                address(this),
                makeAddr("not filtered")
            )
        );
    }
    ...
    function testIsOperatorFiltered_subscription() public {
        ...
        assertFalse(
            registry.isOperatorFiltered(address(this), makeAddr("not filtered"))
        );
    }
    ...
    function testIsOperatorAllowed_NotRegistered() public {
        assertTrue(
            registry.isOperatorAllowed(address(this), makeAddr("allowed"))
        );
    }
    ...
    function testIsOperatorAllowed() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(AddressFiltered.selector, address(operator))
        );
        registry.isOperatorAllowed(address(this), operator);
        vm.expectRevert(
            abi.encodeWithSelector(
                CodeHashFiltered.selector,
                address(toCheck),
                codeHash
            )
        );
