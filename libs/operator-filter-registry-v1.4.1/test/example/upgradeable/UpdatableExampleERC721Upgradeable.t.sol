...
contract TestableUpdatableExampleERC721Upgradeable is
    UpdatableExampleERC721Upgradeable
{
...
contract UpdatableERC721UpgradeableForUpgradableTest is
    BaseRegistryTest,
    Initializable
{
    ...
    function testFilter() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(AddressFiltered.selector, filteredAddress)
        );
    ...
    function testExclusionExceptionDoesNotApplyToOperators() public {
        ...
        vm.startPrank(alice);
        vm.expectRevert(
            abi.encodeWithSelector(AddressFiltered.selector, alice)
        );
...
contract ConcreteUpdatableOperatorFiltererUpgradable is
...
contract UpdatableERC721UpgradeableForUpdatableTest is BaseRegistryTest {
    ...
    function testConstructory_noSubscribeOrCopy() public {
        ...
        assertEq(
            logs[1].topics[0],
            keccak256("RegistrationUpdated(address,bool)")
        );
        assertEq(
    ...
    function testConstructor_copy() public {
        address deployed = computeCreateAddress(
            address(this),
        ...
        assertEq(
            logs[2].topics[0],
            keccak256("OperatorUpdated(address,address,bool)")
        );
        assertEq(
            address(uint160(uint256(logs[2].topics[1]))),
            address(filterer2)
        );
        assertEq(
            logs[3].topics[0],
            keccak256("CodeHashUpdated(address,bytes32,bool)")
        );
        assertEq(
            address(uint160(uint256(logs[3].topics[1]))),
            address(filterer2)
        );
    ...
    function testConstructor_subscribe() public {
        ...
        assertEq(
            logs[2].topics[0],
            keccak256("SubscriptionUpdated(address,address,bool)")
        );
        assertEq(
            address(uint160(uint256(logs[2].topics[1]))),
            address(filterer2)
